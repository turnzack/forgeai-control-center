  ...
  return (
    <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "24px 20px" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <span style={{ fontSize: "10px", fontWeight: 800, color: "#10B981", background: "rgba(217, 70, 239, 0.15)", padding: "2px 8px", borderRadius: 4 }}>
            ASSISTANT VOCAL & IA
          </span>
          <h1 style={{ margin: "4px 0 0 0", fontSize: "22px", color: "#f8fafc" }}>🛒 PACK E-COMMERCE</h1>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button
            onClick={() => setIsRecording(!isRecording)}
            style={{ padding: "8px 16px", borderRadius: 20, background: isRecording ? "#ef4444" : "#10B981", color: "#fff", border: "none", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontWeight: 600 }}
          >
      ...
      <div style={{ background: "rgba(15, 23, 42, 0.6)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 18, height: "400px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        {messages.map(m => (
          <div key={m.id} style={{ display: "flex", gap: 10, alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "80%" }}>
            {m.role === "assistant" && <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#10B981", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}><Bot size={15} /></div>}
            <div style={{ background: m.role === "user" ? "#10B981" : "rgba(255,255,255,0.06)", padding: "10px 14px", borderRadius: 10, fontSize: "13px", color: "#f8fafc" }}>
              <div>{m.content}</div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
        ...
        />
        <button onClick={handleSend} style={{ background: "#10B981", color: "#fff", border: "none", padding: "0 20px", borderRadius: 8, cursor: "pointer", fontWeight: 700 }}>
          <Send size={16} />
        </button>
      </div>
    </div>
